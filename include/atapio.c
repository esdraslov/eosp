...
void ata_read_sectors(uint8_t drive_id, uint32_t lba, uint8_t sectors_count, uint16_t *buffer) {
    ...
    // 1. Send commands (Keep your outb statements here)
    outb(DEVCTRL, 0x2); // tell ATA TO SHUT UP
    outb(base_port + SECCOUNT, sectors_count); // read sectors
    outb(base_port + LBALOW, (uint8_t)lba); // where is that sector
    outb(base_port + LBAMID, (uint8_t)(lba >> 8));
    outb(base_port + LBAHIGH, (uint8_t)(lba >> 16));
    outb(base_port + DVCOMMAND, 0x20); // Read command
    ...
    for (int i = 0;i < 256 * sectors_count; i++)
    {
        buffer[i] = inw(base_port + DATAPORT); // actually read (I FORGOT THIS THE FIRST TIME :/)
    }
...
void ata_write_sectors(uint8_t drive_id, uint32_t lba, uint8_t sectors_count, uint16_t *buffer) {
    ...
    outb(base_port + DHSELECT, 0xE0 | (slave_bit << 4) | ((lba >> 24) & 0x0F));
    outb(base_port + DEVCTRL, 0x2); // SHUT UP ATA
    outb(base_port + SECCOUNT, sectors_count); // Write 1 sector
    outb(base_port + LBALOW, (uint8_t)lba);
    outb(base_port + LBAMID, (uint8_t)(lba >> 8));
    outb(base_port + LBAHIGH, (uint8_t)(lba >> 16));
...
// uint32_t ata_get_total_sectors(uint8_t drive_id) {
...
//     // 2. Send the IDENTIFY command
//     outb(0x1F7, 0xEC);
    
//     // 3. Wait for the drive to process (DRQ sets)
//     while ((inb(0x1F7) & 0x80)) {};
    
//     // 4. Read the 256 words of configuration data
//     uint16_t identify_data[256];
//     for (int i = 0; i < 256; i++) {
//         identify_data[i] = inw(0x1F0);
//     }
    
//     // 5. Combine Word 60 and Word 61 to get the 32-bit sector count
